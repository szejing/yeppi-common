export declare enum CustomerInsightCategory {
    PURCHASE = "purchase",
    SERVICE = "service",
    COMMUNICATION = "communication",
    RISK = "risk",
    PREFERENCE = "preference"
}
export declare enum CustomerInsightConfidence {
    LOW = "low",
    MEDIUM = "medium",
    HIGH = "high"
}
export declare enum CustomerInsightSeverity {
    INFO = "info",
    WARNING = "warning",
    RISK = "risk"
}
export declare enum CustomerInsightSource {
    ADMIN = "admin",
    SYSTEM = "system"
}
export declare enum CustomerInsightKey {
    PRICE_SENSITIVE = "price_sensitive",
    HIGH_INTENT_BUYER = "high_intent_buyer",
    FREQUENT_REFUND = "frequent_refund",
    REQUIRES_FOLLOW_UP = "requires_follow_up",
    COMPLAINT_PRONE = "complaint_prone",
    POLITE = "polite",
    VIP_CANDIDATE = "vip_candidate",
    SUPPORT_SENSITIVE = "support_sensitive",
    AGGRESSIVE_BUYER = "aggressive_buyer"
}
//# sourceMappingURL=customer-insight.d.ts.map